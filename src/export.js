...
import {
  ...
  MODEL_INPUT_COST, DONATION_ADDRESSES, isMainModule
} from './utils.js';

ensureDataDirs();

// \u2500\u2500 Untrusted text \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
// Everything below is built from file and project NAMES, which come from
// whatever repo was open. These reports exist to be shared, so a hostile name
// must not become markup in someone else's browser.

/** Text safe to interpolate into HTML markup. */
...
/**
 * JSON safe to embed inside a <script> block. JSON.stringify does NOT escape
 * `</script>`, so a project named `</script>...` closes the block and whatever
...
export function buildMarkdown(stats, date) {
  ...
  if (stats.topWastedFiles && stats.topWastedFiles.length > 0) {
    ...
    for (const f of stats.topWastedFiles.slice(0, 10)) {
      md += `| \`${escapeMarkdownCell(basename(f.fullPath))}\` | ${formatTokens(f.totalTokensWasted)} | ${f.sessions} |\n`;
    }
    md += '\n';
  }
  ...
  if (stats.topUsefulFiles && stats.topUsefulFiles.length > 0) {
    ...
    for (const f of stats.topUsefulFiles.slice(0, 10)) {
      md += `| \`${escapeMarkdownCell(basename(f.fullPath))}\` | ${f.totalEdits} | ${f.totalReads} | ${f.sessions} |\n`;
    }
    md += '\n';
  }
...
function exportMarkdown() {
  ...
  writeFileSync(outFile, buildMarkdown(stats, date));
  console.log(`Exported to: ${outFile}`);
  return outFile;
}

export function buildHtml(stats, date) {
...
<div class="container">
  <div class="header">
    <h1>Context Optimizer Dashboard</h1>
  ...
  <div class="cards">
    ...
    <div class="card accent">
      ...
      <div class="sub">${formatTokens(stats.estimatedTokensSaved)} / ~$${wastedCostOpus.toFixed(2)}</div>
    </div>
    <div class="card green">
      <div class="label">Saved</div>
      <div class="value">$${wastedCostOpus.toFixed(2)}</div>
      <div class="sub">potential with CCO</div>
    </div>
    <div class="card">
  ...
  <table>
    ...
    ${stats.topWastedFiles.slice(0, 10).map(f => {
      ...
      return `<tr><td><code>${escapeHtml(basename(f.fullPath))}</code></td><td>${formatTokens(f.totalTokensWasted)}</td><td>${f.sessions}</td><td><span class="bar bar-r" style="width:${w}px"></span></td></tr>`;
    }).join('\n    ')}
  </table>` : ''}
  ...
  <table>
    ...
    ${stats.topUsefulFiles.slice(0, 10).map(f => {
      ...
      return `<tr><td><code>${escapeHtml(basename(f.fullPath))}</code></td><td>${f.totalEdits}</td><td>${f.totalReads}</td><td>${f.sessions}</td><td><span class="bar bar-g" style="width:${w}px"></span></td></tr>`;
    }).join('\n    ')}
  </table>` : ''}

  <div class="donate">
    <h3>Saved tokens? Support the project</h3>
    <p>context-optimizer is free and open source. If it saved you money on tokens, consider supporting development with a donation.</p>
  ...
  <footer>
    <span>Generated by <a href="https://github.com/egorfedorov/claude-context-optimizer">claude-context-optimizer</a></span>
    <span>v3.0.0</span>
  </footer>
</div>
...
new Chart(document.getElementById('tokenUsage'), {
  ...
  data: {
    ...
    datasets: [{
      label: 'Tokens (K)',
      data: ${jsonForScript(chartTokens)},
...
new Chart(document.getElementById('projectChart'), {
  type: 'doughnut',
  data: {
    labels: ${jsonForScript(projects.map(([name]) => name))},
    datasets: [{
      data: ${jsonForScript(projects.map(([, d]) => Math.round(d.tokens / 1000)))},
...
function exportHTML() {
  ...
  writeFileSync(outFile, buildHtml(stats, date));
  console.log(`Exported to: ${outFile}`);
  return outFile;
}

// Guarded: importing this module for tests must not write a report into the
// user's exports directory.
