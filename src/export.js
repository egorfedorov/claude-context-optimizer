...
function exportMarkdown() {
  ...
  const outFile = `${EXPORTS_DIR}/report-${date}.md`;
...
function exportHTML() {
  ...
  const sparkData = (stats.sessionHistory || [])
    .filter(s => s.tokensTotal > 0)
    .slice(-30)
    .map(s => s.wastePercent);
...
<div class="container">
  ...
  const outFile = `${EXPORTS_DIR}/report-${date}.html`;
